...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let state = State {
        ...
        //     f64::max(0., 0.75 - (v - na::Vector2::new(PI / 2., 1.)).magnitude()).sqrt()
        // })),
        // ..nope, test with a simpler case with gradient 1 upwards first
        zero: mesh.integrate_cochain(dex::quadrature::Pointwise(|v| v[1])),
    };
    ...
    let velocity_field: dex::Cochain<1, dex::Primal> =
        mesh.integrate_cochain(dex::quadrature::GaussLegendre6(|v, d| {
            ...
            // ..also simpler field here that matches the gradient direction of the 0-cochain
            d[1]
        }));
    ...
    window.run_animation(dv::Animation {
        ...
        step: |state| {
            ...
            // for a 0-form only the first term in the Lie derivative exists
